...
const CheckoutPage: React.FC = () => {
  ...
  const discountCodes: Record<string, number> = {
    HUTRI2024: 0.17, // diskon 17%
    HarvestHub: 0.05, // diskon 5%
    RevoU: 0.1, // diskon 10%
  };
  ...
  useEffect(() => {
    // Perhitungan ulang total harga setelah diskon
    ...
    const updatedTotalCost =
      totalProductPrice + shippingCost + insuranceCost + serviceFee - discount;
  ...
  const applyDiscount = () => {
    ...
    if (discountValue) {
      setDiscount(totalCost * discountValue);
    } else {
      setDiscount(0);
    }
  };
